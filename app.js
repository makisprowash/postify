...
let state = {
  ...
  fbUser: null, activeTab: "url", activeOTab: "copy"
};
...
async function generate(item) {
  try {
    const res = await fetch(`${SCRAPER}/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ vehicle: item }),
      mode: "cors"
    });
    if (!res.ok) {
...
function updatePublishPanel() {
  ...
  if (wrap) {
    wrap.innerHTML = state.fbUser
      ? `<button class="btn-fb-post" onclick="postToFacebook()">Post to Facebook Marketplace</button>`
      : `<div class="fb-note">Connect Facebook above to enable one-click posting</div>`;
  }
}
